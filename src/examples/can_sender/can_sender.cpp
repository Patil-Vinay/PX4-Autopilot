...
class CANHandler {
public:
    CANHandler() : s(-1), vehicle_attitude_sub(-1), vehicle_angular_velocity_sub(-1), mode(0), current_position(0), runLoop(true), positionRead(false) {}
    ...
    void handleReceivedFrame(const struct can_frame &message) {
        ...
        if (message.can_id == expectedResponseId) {
            ...
            if (message.data[message.can_dlc - 1] == 0x3E) {
                ...
                current_position = (message.data[0] << 24) | (message.data[1] << 16) | (message.data[2] << 8) | message.data[3];
                PX4_INFO("Current position read: %ld", (int32_t)current_position);
                float current_position_degrees = (current_position / 524288.0f) *360;
                PX4_INFO("Current position in degrees: %f", (double)current_position_degrees);
                positionRead = false;
    ...
    void processAttitude() {
        ...
        if (updated) {
            ...
            int32_t target_position = (int32_t)((argangle / 360.0f) * 524288.0f);
            sendWriteDataCommandAndWait(CurrentCANID, CMD_IDX_TARGET_ABSOLUTE_POSITION, (int32_t)target_position+(int32_t)attitude_pitch, 0);
            usleep(200);
            startMotion(CurrentCANID);
        }
    }
    ...
    void processAngularVelocity() {
        ...
        if (updated) {
            ...
            sendWriteDataCommandAndWait(CurrentCANID, CMD_IDX_PROFILE_SPEED, (int32_t)angularRate.y, 0);
            usleep(200);
        }
    }
    ...
    int32_t getCurrentPosition() const {
        PX4_INFO("Function Current position: %ld", (int32_t)current_position);
...
static int can_sender_thread_main(int argc, char *argv[]) {
    if (!can_handler.initSocket()) {
        return -1;
    }

    can_handler.readActualPosition(CurrentCANID);
    usleep(500000);

    can_handler.writeOperationMode(CurrentCANID, 3);
    usleep(500000);
    can_handler.writeMotionMode(CurrentCANID, (can_handler.getMode() == 0) ? 1 : 0);
    usleep(500000);
    can_handler.writeProfileAcceleration(CurrentCANID, 200000);
    usleep(500000);
    can_handler.writeProfileDeceleration(CurrentCANID, 200000);
    usleep(500000);
    can_handler.writeProfileSpeed(CurrentCANID, 150000);
    usleep(500000);
    can_handler.enableMotor(CurrentCANID, true);
    usleep(500000);

    can_handler.writeTargetAbsolutePosition(CurrentCANID, can_handler.getCurrentPosition());
    usleep(500000);
    can_handler.startMotion(CurrentCANID);
...
static void usage(const char *reason) {
    ...
    PX4_INFO("usage: can_sender {start|stop|status} {position|velocity} [initial_angle]");
}

extern "C" __EXPORT int can_sender_main(int argc, char *argv[]) {
    if (argc < 2) {
    ...
    if (!strcmp(argv[1], "s")) {
        ...
        if (_is_running) {
            PX4_WARN("can_sender already running");
            return 0;
        }
        if (!strcmp(argv[2], "p")) {
        ...
        daemon_task = px4_task_spawn_cmd("can_sender",
                                         SCHED_DEFAULT,
                                         SCHED_PRIORITY_DEFAULT,
                                         2000,
                                         (px4_main_t)can_sender_thread_main,
                                         (char *const *)argv);

        CANHandler::thread_should_exit = false;
        if (daemon_task < 0) {
            PX4_ERR("Failed to start can_sender task");
            return -1;
        }
        PX4_INFO("can_sender started successfully");
        return 0;
    }

    if (!strcmp(argv[1], "stp")) {
        if (daemon_task < 0) {
            PX4_WARN("can_sender not running");
            return 0;
        }
        ...
        if (can_handler.initSocket()) {
            can_handler.stopMotion(CurrentCANID);
            usleep(500000);
            can_handler.enableMotor(CurrentCANID, false);
            usleep(500000);
            can_handler.closeSocket();
        ...
        PX4_INFO("can_sender stopped");
        return 0;
    }

    if (!strcmp(argv[1], "sta")) {
        if (_is_running) {
            PX4_INFO("can_sender is running");
        } else {
            PX4_INFO("can_sender is not running");
        }
        return 0;
    }
